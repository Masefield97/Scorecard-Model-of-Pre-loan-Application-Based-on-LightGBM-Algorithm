...
# 建模数据及数据对应的数据字典存放路径
path='F:\\DataAna\\策略\\Chapter9\\9.2.2贷前申请评分卡模型开发\\'
...
my_data=my_data[(my_data['apply_mth'].map(lambda x:x>='2020-03' and x<='2022-01'))&(my_data['if_loan_flag']==1)&(my_data['agr_mob12_dpd_30']==1)]

# 删除建模时不需要的字段
my_data.drop(labels=['apply_day', 'apply_week', 'apply_mth', 'product_name','apply_refuse_flag','agr_mob3_dpd_30', 'agr_mob6_dpd_30', 'agr_mob9_dpd_30',
                    'agr_fpd_15', 'mob3_dpd_30_act', 'mob6_dpd_30_act','mob9_dpd_30_act', 'fpd_15_act', 'if_loan_flag','if_loan_in_30', 'agr_mob12_dpd_30'], axis=1, inplace=True)
...
# 6.对训练集数据进行描述性统计分析
train_var_describe=describe_stat_ana(describe_data=trainData,var_dict=var_dict,sample_range='202003-202201',target='mob12_dpd_30_act',seq=1,na_threshold=0.85,vardict_varengname='变量名',vardict_varchiname='变量描述',sample_category='Train')
...
import_vars=pd.merge(import_vars,train_var_describe[['Characteristic','Description']],on='Characteristic',how='left')[['Characteristic','Description', 'Importance']]
import_vars.head()
...
train_model_evaluate=model_evaluate_fun(sample='Train(202003-202201)',data=X_train,prob1='prob',pred1='pred',target='mob12_dpd_30_act')
...
test_var_describe=describe_stat_ana(describe_data=testData,var_dict=var_dict,sample_range='202003-202201',target='mob12_dpd_30_act',seq=1,na_threshold=0.85,vardict_varengname='变量名',vardict_varchiname='变量描述',sample_category='Test')
...
test_model_evaluate=model_evaluate_fun(sample='Test(202003-202201)',data=X_test,prob1='prob',pred1='pred',target='mob12_dpd_30_act')
# 合并模型评估统计量
...
# ROC曲线
plot_roc_fun(train_data=X_train,test_data=X_test,train_label='Train',test_label='Test',prob='prob',target='mob12_dpd_30_act',dpi=150,bg_color='white',linewidth=0.9,save_name=path_report+'all_roc_curve.png')

# PR曲线
plot_pr_fun(train_data=X_train,test_data=X_test,train_label='Train',test_label='Test', prob='prob',target='mob12_dpd_30_act',dpi=150,bg_color='white',linewidth=0.9,save_name=path_report+'all_pr_curve.png')

# KS曲线
plot_ks_combine_fun(train_data=X_train,test_data=X_test,train_label='Train Set KS Curve',test_label='Test Set KS Curve',score='Score',target='mob12_dpd_30_act',dpi=150,bg_color='white',bins=135,linewidth=0.9,save_name=path_report+'all_ks_curve.png')

# 好坏样本对应的模型得分分布图
black_white_dis_combine_plot(train_data=X_train,test_data=X_test,train_label='Train Set Good and Bad distribution',test_label='Test Set Good and Bad Distribution',score='Score',target='mob12_dpd_30_act',dpi=120,bg_color='white',bins=160,save_name=path_report+'all_model_score_dis_curve.png')
